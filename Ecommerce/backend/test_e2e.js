...
const runTests = async () => {
  ...
  try {
    ...
    assert(healthRes.status === 'online' || healthRes.success || healthRes.data?.status === 'UP', 'Health endpoint status is online');
    ...
    const testProduct = productsRes.data?.products?.find((p) => p.stock > 10) || productsRes.data?.products?.[0];
    ...
    assert(reviewRes.success || reviewRes.message, 'Customer photo review successfully handled');
    ...
    const cartSubtotal = getCartRes.data?.subtotal || 1000;
    assert(getCartRes.success && getCartRes.data?.items?.length > 0, `Cart retrieved with subtotal: ₹${cartSubtotal}`);

    const couponRes = await fetch(`${BASE_URL}/payment/validate-coupon`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: 'SAVE10', order_amount: cartSubtotal })
    }).then((r) => r.json());
    const discountVal = couponRes.data?.discount_amount || couponRes.data?.discountAmount || couponRes.data?.discount || 0;
    assert(couponRes.success && discountVal > 0, `Coupon "SAVE10" applied (Saved: ₹${discountVal})`);
    ...
    assert(createPayRes.success && createPayRes.data?.order_id, `Payment Gateway Order created: ${createPayRes.data?.order_id} (Mode: ${createPayRes.mode || 'gateway'})`);
    ...
    const orderNumber = orderRes.data?.order_number || orderRes.data?.orderNumber || `ORD-${Date.now()}`;
    assert(orderRes.success && orderRes.data, `Order #${orderNumber} placed successfully`);
    ...
    assert(webhookRes.status === 'ok' || webhookRes.success, 'Payment webhook listener verified and processed successfully');
    ...
    assert(adminStatsRes.success && adminStatsRes.data?.metrics?.totalOrders >= 0, `Admin Dashboard loaded: Turnover ₹${adminStatsRes.data?.metrics?.totalRevenue}`);

    // 11. Brands Spotlight & Catalog Metadata
    const brandsRes = await fetch(`${BASE_URL}/products/brands`).then((r) => r.json());
    assert(brandsRes.success && brandsRes.data?.length > 0, `Dynamic Brands API returned ${brandsRes.data?.length} brands with metadata`);
    ...
    assert(newsletterRes.success, 'Newsletter subscribed successfully');
